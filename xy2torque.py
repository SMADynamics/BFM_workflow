...
class XY_2_Torque():
    ...
    def workflow(self):
        ...
        if self.rm_drift :
            print(f'workflow(): removing drift {self.rm_drift_mode}')
            ...
            print('workflow(): drift NOT removed.')
            x_rmdrift = x
            y_rmdrift = y
        
        # 2) stretch the elliptical trajectory (x(t),y(t)) into a circle:
        if self.stretch_xy:
            print('workflow(): stretching to circle')
            self.x_circ, self.y_circ = ellipse_fit.stretch_ellipse(x_rmdrift, y_rmdrift, plots=self.stretch_xy_plots)
        else:
            print('workflow(): NOT stretching to circle')
            self.x_circ = x_rmdrift
            self.y_circ = y_rmdrift
        ...
        print(f'workflow(): Done angle(t), speed(t) traj_radius(t), drag(t)')
    
        # 6) calculate the torque trace:
        self.torque_pNnm = self.drag_pNnms[:-1] * self.speed_Hz*2*np.pi
        print(f'workflow(): filtering torque trace by {self.filter_name} win:{self.filter_win}')
